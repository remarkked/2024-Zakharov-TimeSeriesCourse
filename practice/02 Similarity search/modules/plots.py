...
def plot_bestmatch_results(ts: np.ndarray, query: np.ndarray, bestmatch_results: dict):
    """
    ...
    for i in range(len(bestmatch_results['indices'])):
        fig.add_trace(go.Scatter(x=np.arange(query_len) + bestmatch_results["indices"][i], y=ts[bestmatch_results["indices"][i]:bestmatch_results["indices"][i]+query_len], line=dict(color=px.colors.qualitative.Plotly[1])),
                row=1, col=2)
